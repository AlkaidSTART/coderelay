...
export async function loadConfig(options: LoadConfigOptions = {}): Promise<LoadedConfig> {
  ...
  if (!path) {
    if (options.allowMissing === false) {
      throw new ConfigError(
        `no config file found from ${cwd}; run "coderelay init" or create ${CONFIG_DIR}/config.yaml`,
      );
    }
  ...
  try {
    ...
    throw new ConfigError(`invalid YAML in ${path}: ${(error as Error).message}`, { path, cause: error });
  }
...
export async function resolveConfigPath(cwd = process.cwd()): Promise<string | null> {
